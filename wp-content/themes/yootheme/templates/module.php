...
if (in_array($position, ['navbar', 'navbar-push','navbar-mobile', 'header-mobile']) ||
    ...
    // Search
    if (in_array($module->type, ['search', 'finder']) && ((!str_ends_with($position, '-mobile') && $config("$header.search_style") == 'modal') || (str_ends_with($position, '-mobile') && $config("$mobile.header.search_style") == 'modal'))) {
        echo $module->content;
        return;
    // Else
    } else {
...
// Class
if ($cls = (array) $config("$modules.class")) {
...
if (preg_match('/^(top|bottom|sidebar|dialog(-push|-mobile(-push)?)?|builder-\d+)$/', $position)) {
